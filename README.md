План разговора:
- [x] План действий по проекту — сегодня разбираем Git, личные комментарии — до понедельника, а в понедельник — двигаемся в Docker
  * Оговорка про первый проект
  * Просьба отправить имя и фамилию с гитом в Pumble
- [ ] Разбор гита
  - [x] Как работают пулреквесты и отправка данных из одного репозитория в другой и как их правильно отправлять
  - [x] Оформление коммитов и пулреквестов
    - [x] Отсутствие мусора в коде: 
      - [x] комментарии c мертвым кодом, 
      - [x] `console.log`, 
      - [x] изменения в стиле кода
    - [x] Атомарность коммитов и ~~пулреквестов~~: один коммит ~~и один пулреквест~~ — одна задача
    - [x] Название коммитов
      - [x] Начинайте сразу с сути — что было сделано в коммите
      - [x] Уберите контексты (сделал домашку, изменил файл такой-то)
    - [x] Название пулреквестов
- [x] Вопросы
  - [x] Вопрос про `target`
  - [x] Вопрос про перерисовку страниц

## `target`
Как сделать так, чтобы окно закрывалось по клику вне его

Через проверку содержится ли `Event.target` внутри элемента вне которого
мы кликаем
...
### Вопросы которые нужно задать
  1. Можно ли получить доступ к другим элементам? Если вы показываете попап — то тогда переход на отдельную страницу не нужен
  2. Нужно ли этой страницей пользоваться с отключенным JS?

### Современная схема построения приложений
Router — доступ к адресной строке
  / - Main
  /news - News
+ SSR — Server-Side Rendering
  - загрузка данных — на стороне сервера до первичного рендера
  - отправка данных — через отправку формы
